(function($, window) {
	...
	var itemTemplate = '<div class="mui-slider-item mui-zoom-wrapper {{className}}"><div class="mui-zoom-scroller"><img src="{{src}}" data-id="{{dataId}}" data-name="{{dataName}}" data-preview-lazyload="{{lazyload}}" style="{{style}}" class="mui-zoom"></div></div>';
	...
	var PreviewImage = function(options) {
		this.options = $.extend(true, {
			...
			zoom: true,
			header: '<span class="mui-preview-indicator"></span><span class="mui-preview-title"></span><span class="mui-preview-btns"></span>',
			footer: ''
		}, options || {});
		this.init();
		this.initEvent();
	...
	proto.init = function() {
		...
		this.imgTitle = this.element.querySelector($.classSelector('.preview-title'));
		this.imgBtns = this.element.querySelector($.classSelector('.preview-btns'));
		...
		if(options.btns && options.btns.length){
			...
			for(var i=0;i<options.btns.length;i++){
				...
				btn.className = options.btns[i].iconCls;
				btn.setAttribute('index',i);
				this.imgBtns.appendChild(btn);
				btn.addEventListener('tap',function(evt){
					var item = me.scroller.querySelector($.classSelector('.slider-item') + $.classSelector('.active'));
					var index = this.getAttribute('index');
					if(typeof options.btns[index].handler == 'function'){
						options.btns[index].handler.call(this,item.querySelector('img'));
					}
				},false);
			}
		}
	...
	proto.initEvent = function() {
		var self = this;
		$(document.body).on('tap', 'img[data-preview-src]', function(evt) {
			self.open(this);
			return false;
		});
		var laterClose = null;
	...
	proto._loadItem = function(index, isOpening) { //TODO 暂时仅支持img
		...
		this.imgTitle && (this.imgTitle.innerHTML = itemData.name);
	...
	proto.refresh = function(index, groupArray) {
		...
		for (var i = 0; from < to; from++, i++) {
			...
			itemStr = itemTemplate.replace('{{src}}', itemData.src).replace('{{lazyload}}', itemData.lazyload)
				.replace('{{style}}', style)
				.replace('{{dataId}}',itemData.id)
				.replace('{{dataName}}',itemData.name);
			if (from === index) {
				currentIndex = i;
		...
		this.imgTitle && (this.imgTitle.innerHTML = groupArray[currentIndex].name);
	...
	proto.open = function(index, group) {
		...
		if(this.options.onOpen){
			this.options.onOpen.call(this,index,group);
		}
	...
	proto.close = function(index, group) {
		...
		if(this.options.onClose){
			this.options.onClose.call(this,index,group);
		}
